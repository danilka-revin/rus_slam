...
- **Расчет несущей способности под полезную нагрузку 250 кг:**
  ...
  - Нагрузка равномерно распределяется между 4 поворотными опорами ($N \approx 710 \text{ Н} \approx 72.4 \text{ кг}$ на опору).
  - Момент сопротивления профиля 40×40×2 мм составляет $W \approx 3.71 \text{ см}^3$. При максимальном расчетном изгибающем моменте на продольных балках базы (длина пролета 550 мм) максимальные напряжения изгиба в металле составляют менее $36 \text{ МПа}$, что дает **более чем 6-кратный запас прочности** относительно предела текучести стали Ст3сп ($\sigma_T \approx 235 \text{ МПа}$).
